...
def check_overlap(db: Session, model_class, start_date: str, end_date: str, exclude_id: Optional[int] = None) -> bool:
    """Check if a goal would overlap with existing goals."""
    query = db.query(model_class).filter(
        model_class.start_date <= end_date,
        model_class.end_date >= start_date
    )
    if exclude_id:
...
def get_sprint_for_date(date: str, db: Session = Depends(get_db)):
    ...
    goal = db.query(models.SprintGoal).filter(
        models.SprintGoal.start_date <= date,
        models.SprintGoal.end_date >= date
    ).first()
    ...
    if not goal:
        goal = db.query(models.SprintGoal).filter(
            models.SprintGoal.start_date > date
        ).order_by(models.SprintGoal.start_date).first()
...
@router.post("/sprint", response_model=schemas.GoalResponse)
...
def update_sprint_goal(goal_id: int, goal_update: schemas.GoalUpdate, db: Session = Depends(get_db)):
    ...
    if check_overlap(db, models.SprintGoal, new_start, new_end, exclude_id=goal_id):
        raise HTTPException(
            status_code=400, 
            detail=f"A sprint goal already exists that overlaps with {new_start} to {new_end}"
        )
...
def get_quarterly_for_date(date: str, db: Session = Depends(get_db)):
    ...
    # First try to find active goal (date is within range)
    goal = db.query(models.QuarterlyGoal).filter(
        models.QuarterlyGoal.start_date <= date,
        models.QuarterlyGoal.end_date >= date
    ).first()
    ...
    if not goal:
        goal = db.query(models.QuarterlyGoal).filter(
            models.QuarterlyGoal.start_date > date
        ).order_by(models.QuarterlyGoal.start_date).first()
...
@router.post("/quarterly", response_model=schemas.GoalResponse)
...
def update_quarterly_goal(goal_id: int, goal_update: schemas.GoalUpdate, db: Session = Depends(get_db)):
    ...
    if check_overlap(db, models.QuarterlyGoal, new_start, new_end, exclude_id=goal_id):
        raise HTTPException(
            status_code=400, 
            detail=f"A quarterly goal already exists that overlaps with {new_start} to {new_end}"
        )
