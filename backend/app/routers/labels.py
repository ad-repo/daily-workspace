...
def search_by_label(
    label: Optional[str] = Query(None, description="Label name to search for"),
    ...
    notes_with_label = db.query(models.DailyNote).filter(
        models.DailyNote.labels.contains(label_obj)
    ).all()
    
    # Get all entries with this label and their parent notes
    entries_with_label = db.query(models.NoteEntry).filter(
        models.NoteEntry.labels.contains(label_obj)
    ).all()
    
    # Get unique notes from entries
    note_ids_from_entries = set(entry.daily_note_id for entry in entries_with_label)
    notes_from_entries = db.query(models.DailyNote).filter(
        models.DailyNote.id.in_(note_ids_from_entries)
    ).all()
